...
const convertPostgresSQLResponseToColumnMetaData = rows => {
  if (rows && rows.length > 0) {
    return rows.map(row => {
      ...
      const typeName = (_Object$keys$find = Object.keys(_pg.types.builtins).find(d => _pg.types.builtins[d] === row.dataTypeID)) !== null && _Object$keys$find !== void 0 ? _Object$keys$find : 'UNKNOWN';
